...
class RandomAgent(PGAgent):
    ...
    def policy(self, state):
        ...
        '''
        # values = np.random.random(size=2)
...
class LinearAgent(PGAgent):
    ...
    def __init__(self, n_states, n_actions):
        ...
        self.policy_parameters = np.random.rand(
            self.states * self.actions)*2-1
        self.c = 1000
        self.alpha = 1/self.c
    ...
    def policy(self, states):
        ...
        '''
        left = states@self.policy_parameters[:4]
        right = states@self.policy_parameters[4:]
        temp = np.array([left, right])
        softmax = np.exp(temp)/np.sum(np.exp(temp), axis=0)
        return softmax
    ...
    def gradient_log_policy(self, state, action):
        ...
        '''
        gradient = []
        grad = self.policy_parameters
        actions = self.policy(state)

        for i in range(self.states):
            # Action Left
            if action == 0:
                aux = state[i] - (state[i] * actions[0] + 0)
            # Action Right
            else:
                aux = 0 - (state[i] * actions[0] + 0)
            gradient.append(aux)

        for i in range(self.states, self.states*self.actions):
            i = i % self.states
            # Action Left
            if action == 0:
                aux = 0 - (0 + state[i] * actions[1])
            # Action Right
            else:
                aux = state[i] - (0 + state[i] * actions[1])
            gradient.append(aux)
    ...
    def update_policy(self, grads, rewards):
        ...
        for i in range(len(grads)):
            update = [self.alpha*g *
                      np.sum(rewards[i:]) for g in grads[i]]
            self.policy_parameters = [sum(x)
                                      for x in zip(self.policy_parameters, update)]
...
class CartPole:
    ...
    def run_episode(self):
        ...
        while True:
            ...
            # Compute gradient
            grad = self.agent.gradient_log_policy(
                state, action)
...
if __name__ == "__main__":
    ...
    print("Training Min", min(cart.episode_rewards))
    print("Training Max", max(cart.episode_rewards))
    print("Training AVG", sum(cart.episode_rewards)/len(cart.episode_rewards))
    plt.plot(cart.test_episode_rewards, 'r')
    print("Test Min", min(cart.test_episode_rewards))
    print("Test Max", max(cart.test_episode_rewards))
    print("Test AVG", sum(cart.test_episode_rewards) /
          len(cart.test_episode_rewards))
